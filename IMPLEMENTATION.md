...
> real, already-running PowerPoint. Still open: the four **Spike** items below
> (need a Windows+PowerPoint session to verify against real COM — the code
> currently encodes the spec's assumed answers), CI, and the v0.1+ stages.
...
- [ ] **UndoRecord absence.** Confirm there is *no* undo-grouping primitive
  (`Application.UndoRecord` or newer) in the installed build. If confirmed,
  `edit()` is view/Selection-only and docs say so. *Assumed:* none exists —
  `_edit.EditScope` opens no UndoRecord and docs/docstrings say so loudly.
  *Finding:* pending real-COM session.
- [ ] **`Visible` behaviour.** Confirm `Application.Visible = False` raises (so
  ...
  *Finding:* pending.
- [ ] **Selection round-trip.** Snapshot `ActiveWindow.View.Slide.SlideIndex` +
  ...
  back to `Unselect()` on any failure. *Finding:* pending (does
  `Shapes.Range([names]).Select()` round-trip?).
- [ ] **Notes placeholder resolution.** Confirm the notes body resolves by
  ...
  `PlaceholderFormat.Type == BODY`, not by index. *Finding:* pending.
...
## `exec` batch ops (lands with v0.1+, gated on the undo decision)

Same `{"label", "ops":[…]}` shape as wordlive, **minus atomic undo** — a 5-op
script is 5 Ctrl-Z entries, applied in order; on failure at op K, ops 1..K-1 are
already applied (report the failing index, re-raise so the exit code maps). No
`"tracked"` key (PowerPoint has no Track Changes). Proposed op set: `add_slide`,
...
2. ~~**The undo gap — biggest call.** Which mitigation is the documented `exec`
   default?~~ **Resolved: per-op undo + honest docs.** Each mutation is its own
   Ctrl-Z; we document that plainly and don't build snapshot/save-point
   machinery. "It is what it is" — the user keeps full Ctrl-Z, just N steps
   not 1.
