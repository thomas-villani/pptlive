...
- [x] **Spike RESOLVED (2026-06-07, `scripts/findreplace_spike.py`, net-zero)** —
  ...
  and adds the fuzzy matching native Find lacks.
...
before hardening; the spike findings are recorded in this section and the
`scripts/*_spike.py` harnesses.
...
- [x] **Library + CLI (`shape set-link`/`remove-link`, `slide set-transition`/
  ...
  exercise on a real deck confirmed.
...
Four roadmap items chosen as a batch and **de-risked together first** by
