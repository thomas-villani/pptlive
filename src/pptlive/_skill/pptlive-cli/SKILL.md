...
- `pptlive set-paragraphs --anchor-id ph:4:body --paragraphs '["First", {"text":"Second","list_type":"bulleted","indent_level":2,"bold":true,"color":"#2E74B5"}]'` — rewrite an anchor as a clean per-paragraph list. The **safe** way to author a bullet list — each item is exactly one `para:`, no `\n` inference. `--file PATH` reads the JSON instead.
  - Item keys, **complete** — `text` (required) · `list_type` (`bulleted`/`numbered`/`none`) · `bullet_char` · `alignment` · `indent_level` (1-5) · `space_before`/`space_after` (**points**) · `space_before_lines`/`space_after_lines` (**multiples**) · `line_spacing` (a **multiple**) · `line_spacing_points` (**exact points**) · `force` · `bold`/`italic`/`underline` · `size` · `font` · `color` (the **font** color).
  - Set font *and* structure in this one call — a follow-up `format-text` per paragraph is 2-3x the COM round-trips for no gain. An unknown key is an error (exit 1), not a silent no-op.
...
- `pptlive read text-frame-status --anchor-id shape:4:3` — autofit/wrap/anchor/margin diagnostics (`autosize`, `word_wrap`, `vertical_anchor`, `margins`, `overflow_risk`) when text looks clipped.
- `pptlive shape set-text-frame --anchor-id shape:4:3 --autosize none --margins 0 [--wrap|--no-wrap] [--vertical-anchor middle] [--margin-left N]` — the **setter** half of that read. Two things matter for precise layout: **`--autosize none`** (a new textbox grows to fit its text, so a `--height` you pass is advisory until you turn autofit off) and **`--margins 0`** (PowerPoint's 0.1in / 7.2pt inner margins silently eat your padding math). `shape add --kind textbox|shape` takes `--autosize`/`--wrap`/`--vertical-anchor`/`--margins` too, so create it already pinned.
...
- **When text overflows,** read `text-frame-status`: `overflow_risk: "possible"` means autosize is off (text can clip); `"low"` means an autofit mode is active. Act on it with `shape set-text-frame`.
- **A textbox's `--height` is advisory until you turn autofit off.** `shape add --kind textbox` creates a frame that grows to fit its text, so your layout drifts as text lands. Pass `--autosize none` (at add time or via `set-text-frame`) whenever the geometry matters, and `--margins 0` so the 7.2pt inner default doesn't eat your padding.
...
- `pptlive media add --slide 4 --kind video --path demo.mp4` — same, but the clip stays visible: `--hide-icon`/`--no-hide-icon` is **audio-only** and passing it here is a usage error, not a silent no-op. Reads carry a `media` field (`{type, length_s, start_s, end_s, muted, volume, autoplay}`); `type` is `sound`/`movie`, `start_s`/`end_s` are the trim window.
...
- `pptlive export-video PATH [--resolution 720] [--fps 30] [--quality 85] [--default-slide-duration 5] [--no-use-timings]` — export the deck to MP4. A **read** (no rebind). Wraps PowerPoint's async CreateVideo: **blocks until done by default** (raises after `--timeout` s, default 600). Returns `{ok, path, status, status_code}`. `--use-timings` (default on) honors per-slide timings + narration; `--default-slide-duration` is **whole seconds** (a fractional value is rejected, not truncated).
...
- **Crop a picture to a box** (the full-bleed verb): `pptlive shape crop-to-fit --anchor-id shape:4:3 --left 0 --top 0 --width 960 --height 540` — `--fit cover` (default) fills the box exactly and centre-crops the overflow; `--fit contain` shrinks the whole picture to fit and centres it (letterboxed). Box args default to the picture's current geometry; existing crops are cleared first, so re-fitting never compounds. **Use this instead of oversizing a picture and letting it hang off the slide** — that would flag `off_slide` in `slide geometry` forever, killing your cheapest pre-render lint. Raw form: `pptlive shape crop --anchor-id shape:4:3 --left 36` (points off one edge, `0` un-crops it) — but note **cropping shrinks the shape box**, so follow with `shape resize` if the box matters. Cropped pictures carry a `crop` field in reads.
