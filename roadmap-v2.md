# pptlive — roadmap v2 (remaining work)

Audit date: 2026-06-14. This file replaces the old forward-looking `roadmap.md`
with **only the work still left to do**.

Audit baseline:

- `roadmap.md` is now mostly historical: v1.0, v1.1, v1.2, v1.3, v1.6, plus
  the quick-win cuts for hyperlinks, transitions, and per-slide solid backgrounds
  are implemented.
- Fake-COM/unit coverage is healthy: `uv run pytest -q` passes (`742 passed, 2
  deselected`).
- Source audit found no shipped implementation yet for the remaining large
  surfaces below (`SectionProperties`, `HeadersFooters`, whole-shape animations,
  media insertion/video export, tags, document properties, OLE, grouping,
  connectors, etc.).
...
| ---- | ----- | ------ | -------------- |
| **v1.4-rest** | Navigation & structure: sections, headers/footers | `[ ]` | Completes deck organization / numbering after shape hyperlinks shipped. |
| **v1.5-rest** | Animations | `[~]` | Spike proved the common whole-shape animation cut round-trips; build is ready. |
| **v1.7** | Media + narrated-video export | `[~]` | Spike proved the high-ceiling workflow: insert narration, self-time slides, export MP4. |
