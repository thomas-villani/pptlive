...
| Tool | `op`s |
| ---- | ----- |
| `ppt_read` | `status` · `slides` · `outline` · `slide` · `anchor` · `selection` · `find` · `table` · `chart` · `smartart` · `comments` · `theme` · `master` · `layouts` · `text_frame_status` (autofit / wrap / margins / overflow-risk) — every read; never moves the view |
| `ppt_edit` | `write` · `set_paragraphs` (rewrite an anchor as a clean per-paragraph list — the safe bullet path) · `find_replace` · `format` (font + paragraph + shape **fill / line** + list bullets in one op; `line_spacing` is a multiple, `line_spacing_points` is exact points) · `text_reset_format` / `shape_reset_layout` (recover a wrecked placeholder) · `slide_add` / `slide_delete` / `slide_duplicate` / `slide_move` / `set_layout` · `shape_add` / `shape_move` / `shape_resize` / `shape_delete` / `shape_order` (z-order) / `set_alt` · `shape_gradient_fill` / `shape_picture_fill` / `shape_pattern_fill` (advanced fills) / `shape_set_effect` (shadow/glow/soft-edge/reflection) · `shape_set_hyperlink` / `shape_remove_hyperlink` · `slide_set_transition` / `slide_set_background` · `table_add_row` / `table_delete_row` · `chart_set_type` / `chart_set_data` / `chart_recolor_text` · `smartart_set_nodes` / `smartart_recolor_text` · `comment_add` / `comment_reply` / `comment_delete` · `theme_set_color` / `theme_set_font` · `master_format_text_style` / `master_format_paragraph_style` / `master_set_background` — every mutation; one Ctrl-Z each |
